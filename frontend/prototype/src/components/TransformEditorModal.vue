...
const output = ref<{
  result?: unknown
  console?: string
  error?: { msg: string; line: number | null; hint: string }
} | null>(null)
...
function toJS(o: unknown): unknown {
  if (o == null) return o
  if (isPyProxy(o)) {
    const c = o.toJs({ dict_converter: Object.fromEntries })
    o.destroy()
    return toJS(c)
  }
  if (o instanceof Map) {
    const r: Record<string, unknown> = {}
    o.forEach((v, k) => (r[String(k)] = toJS(v)))
    return r
  }
  if (Array.isArray(o)) return o.map(toJS)
  if (typeof o === 'object') {
    const r: Record<string, unknown> = {}
    for (const [k, v] of Object.entries(o)) r[k] = toJS(v)
    return r
  }
  return o
}
...
function getHint(errType: string, errMsg: string): string {
  ...
  if (msg.includes('expected an indented block'))
    return 'Add indented code after if/for/def/while statement.'
  ...
  if (msg.includes("'nonetype'") || msg.includes('nonetype'))
    return 'A variable is None. Check function returns a value.'
  ...
  if (msg.includes('unsupported operand'))
    return 'Cannot perform this operation between these types.'
  if (msg.includes('takes') && msg.includes('argument'))
    return 'Wrong number of arguments passed to function.'
  if (msg.includes('object is not callable'))
    return 'Trying to call something that is not a function.'
...
async function handleRun() {
  ...
  try {
    ...
    if (res.error) {
      ...
      output.value = {
        console: res.stdout,
        error: { msg: `${type}: ${msg}`, line: validLine, hint: getHint(type, msg) },
      }
...
function handleDebug() {
  if (!hasRun.value) {
    UIkit.notification({ message: 'Run the code first', status: 'warning', pos: 'top-center' })
    return
  }
...
function setMarker(line: number, msg: string) {
  const ed = editorRef.value?.editor as
    | {
        getModel: () => { getLineCount: () => number; getLineLength: (n: number) => number } | null
  ...
  monaco.editor.setModelMarkers(model, 'err', [
    {
      startLineNumber: ln,
      startColumn: 1,
      endLineNumber: ln,
...
function formatOutput(v: unknown, indent = 2): string {
  ...
  try {
    return typeof v === 'object' ? JSON.stringify(v, null, indent) : String(v)
  } catch {
    return String(v)
  }
}
...
const tableRows = computed(() =>
  outputType.value === 'table' ? (output.value?.result as Record<string, unknown>[]) : [],
)
</script>
    ...
    <div v-if="show" class="te-overlay" :class="{ dark: isDark }" @click.self="handleCancel">
      <div class="te-modal" :class="{ dark: isDark }">
        <div class="te-header">
          ...
          <span
            class="te-status"
            :class="{ loading: pyodideLoading, ready: pyodideReady && !pyodideLoading }"
          >
          ...
          <div v-if="hasRun" class="te-output">
            ...
            <div class="te-output-content" :class="{ dark: isDark }">
                  ...
                  <table v-if="outputType === 'table'" class="te-table">
                    ...
                    <tbody>
                      <tr v-for="(row, i) in tableRows" :key="i">
                        <td v-for="c in tableCols" :key="c">{{ formatOutput(row[c], 0) }}</td>
                      </tr>
                    </tbody>
                  </table>
                  <pre v-else-if="outputType === 'json'" class="json">{{
              ...
              <div v-else class="error">
                <div class="error-line" v-if="output.error.line">
        ...
        <div class="te-footer">
          ...
          <div class="te-footer-right">
            ...
            <button class="te-btn te-btn-debug" :disabled="isExecuting" @click="handleDebug">
              🔍 Debug
            </button>
